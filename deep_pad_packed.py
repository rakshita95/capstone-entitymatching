...
# -*- coding: utf-8 -*-
"""
Created on Wed May  9 21:40:47 2018
...
max_desc1_len = [len(x) for x in merged['description_x']]
max_desc2_len = [len(x) for x in merged['description_y']]
...
class Transform(Dataset):
    ...
    def __getitem__(self, index):
        ...
        desc1_idx = [self.word2idx.get(word, 1) for word in desc1]
        desc2_idx = [self.word2idx.get(word, 1) for word in desc2]
        ...
        return LongTensor(desc1_idx), desc1_len, LongTensor(desc2_idx), desc2_len, sim, label
...
def module_fn(batch_len, desc_words, max_len):
    desc_words_padded = LongTensor(batch_len, max_len).zero_()  # N X max_len

    for i in range(batch_len):
        if len(desc_words[i].size())!=0:
            desc_words_padded[i, 0:desc_words[i].size(0)].copy_(desc_words[i])

    return desc_words_padded
...
def pad_batch(batch):
    ...
    for val in batch:
        desc1_words.append(val[0])
        # if len(val[0].size()) == 0:
        #     length_desc1.append(0)
        # else:
        #     length_desc1.append(val[0].size(0))
        length_desc1.append(val[1])
        desc2_words.append(val[2])
        length_desc2.append(val[3])
        # if len(val[2].size()) == 0:
        #     length_desc2.append(0)
        # else:
        #     length_desc2.append(val[2].size(0))
        sim.append(val[4])
        actual.append(val[5])
    print(max(length_desc2))
    return (*module_fn(len(batch), desc1_words, max(length_desc1))), \
           length_desc1,\
           (*module_fn(len(batch), desc2_words, max(length_desc2))), \
           length_desc2, sim, actual
...
train_loader = torch.utils.data.DataLoader(
    ...
    pin_memory=True, collate_fn=pad_batch)
...
def encode(rnn, seq, lengths):
    key = sorted(range(len(lengths)), key=lambda k: lengths[k], reverse=True)

    seq = seq.index_select(0,
                           Variable(torch.LongTensor(key))).transpose(0, 1)
    rnn_input = nn.utils.rnn.pack_padded_sequence(seq, sorted(lengths,
                                                              reverse=True))
    # rev_key = sorted(range(len(key)), key=lambda k: key[k])
    rnn_input = nn.utils.rnn.PackedSequence(rnn_input.data,
                                            rnn_input.batch_sizes)
    # seq_len, batch, input_size
    output = rnn(rnn_input)[1][0].squeeze()
    # output = nn.utils.rnn.pad_packed_sequence(output)[0].transpose(0,
    #                                                                1).index_select(
    #     0, Variable(torch.LongTensor(rev_key).cuda()))
    print(output.size())
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.rnn2 = nn.LSTM(input_size=300, hidden_size=50) # for entity1
    ...
    def forward(self, desc1_embed, desc1_len, desc2_embed, desc2_len, sim):

        # matcher(desc1_embed, desc1_len, desc2_embed, desc2_len, sim)

        batch_size = desc1_embed.size()[0]
        desc1_encoded = encode(self.rnn1, desc1_embed, desc1_len)
        desc2_encoded = encode(self.rnn2, desc2_embed, desc2_len)
        # desc1_encoded = self.rnn1(desc1_embed.transpose(1, 0))[1][
        #     0].squeeze()  # .view(batch_size, -1)
...
for epoch in range(0, num_epochs):
    ...
    for idx, sample in enumerate(train_loader):
        ...
        desc1_len = Variable(sample[1])
        desc2_idx = Variable(sample[2])
        desc2_len = Variable(sample[3])
        sim = Variable(sample[4])
        actual = Variable(sample[5])
        print(desc1_idx.size())
        print(desc2_idx.size())
        if idx>4:
            break
        ...
        pred = matcher(desc1_embed, desc1_len, desc2_embed, desc2_len, sim)
    ...
    val_acc, val_f1, val_recall = validate(dev_loader, reader)
    train_acc, train_f1, train_recall = validate(train_loader, reader)
    print(val_acc)
    print(train_acc)
